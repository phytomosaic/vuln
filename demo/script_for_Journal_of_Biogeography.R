...
#  Rob Smith, smithr2@oregonstate.edu, Oregon State Univ, 03 Sep 2019
...
pdf('./fig/S4_nichebreadth.pdf', wid=cm_to_in(17), hei=cm_to_in(15))
set_par(6)
par(mfrow=c(2,3), oma=c(0,1,3,0.3), mar=c(4,3,0,0), mgp=c(2.0,0.6,0))
...
### ------- Figure 01 - conceptual figure -------
pdf('./fig/JBI-19-0036_Fig_01_concept.pdf',
    wid=cm_to_in(8), hei=cm_to_in(16))
par(mfrow=c(2,1), oma=c(0,0,1,0), mar=c(2.8,2.8,0.5,0), mgp=c(1.7,0.5,0),
...
### ------- Figure 02 - truncated vs extended niches -------
...
pdf('./fig/JBI-19-0036_Fig_02_truncation.pdf',
    wid=cm_to_in(17), hei=cm_to_in(11))
...
pdf('./fig/JBI-19-0036_Fig_03_mapvuln.pdf',
    wid=cm_to_in(17), hei=cm_to_in(8.5))
...
grid.arrange(
    ...
    mmap(tmp, 't11', name=lab1, alpha=tmp$t11/100, sizept=.5, sizetxt=.7),
    mmap(tmp, 't12', name=lab2, alpha=tmp$t12/100, sizept=.5, sizetxt=.7, col='bw2', midval=60),
    mmap(tmp, 't1c', name=lab3, sizept=.5, sizetxt=.7),
    grid::textGrob('CMD', rot=90, gp=gp),
    mmap(tmp, 't21', name=lab1, alpha=tmp$t21/100, sizept=.5, sizetxt=.7),
    mmap(tmp, 't22', name=lab2, alpha=tmp$t22/100, sizept=.5, sizetxt=.7, col='bw2', midval=60),
...
pdf('./fig/JBI-19-0036_Fig_04_elev-latitude.pdf',
    wid=cm_to_in(17), hei=cm_to_in(9.5))
set_par(6)
par(oma=c(0,1,1.5,0))
...
p20 <- mmap(tmp, nm[pk[1]], '', titl[1], tmp[,pk[1]]/100, .5, .7, col='bw2', midval=70) + mm
p21 <- mmap(tmp, nm[pk[2]], '', titl[2], tmp[,pk[2]]/100, .5, .7, col='bw2', midval=70) + mm
p22 <- mmap(tmp, nm[pk[3]], '', titl[3], tmp[,pk[3]]/100, .5, .7, col='bw2', midval=70) + mm
p23 <- mmap(tmp, nm[pk[4]], '', titl[4], tmp[,pk[4]]/100, .5, .7, col='bw2', midval=70) + mm
p24 <- mmap(tmp, nm[pk[5]], '', titl[5], tmp[,pk[5]]/100, .5, .7, col='bw2', midval=70) + mm
...
pdf('./fig/JBI-19-0036_Fig_05_warming.pdf',
    wid=cm_to_in(17), hei=cm_to_in(20))
...
pdf('./fig/JBI-19-0036_Fig_06_veryvuln.pdf',
    wid=cm_to_in(8), hei=cm_to_in(8))
